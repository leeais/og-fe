...
import type { InstructorFormValues } from "./utils";
import { useModal } from "@/hooks/useModal";

interface ModalInstructorProps {
  name: string;
}

export default function ModalInstructor({ name }: ModalInstructorProps) {
  const { closeModal } = useModal();
  const [form] = Form.useForm();

  function handleSubmit(values: InstructorFormValues) {
    try {
      console.log(values);
      form.resetFields();
      closeModal();
      ...
      <Form className="space-y-2" onFinish={handleSubmit} form={form} layout="vertical">
        ...
        <div className="space-y-2 p-2 rounded border border-border">
          ...
          <div className="flex items-center gap-2">
            ...
            <span>hoặc</span>
            <Form.Item className="flex-1 m-0" name="InstructorId">
